...
`std::is_nothrow_invocable_r`の{{linkof('yu::tuples::apply')}}版である．


## 効果

Tupleを展開して型`F`のオブジェクトを関数呼び出しする式が有効で，その戻り値型が`R`に変換可能であり，さらにその際に例外を送出しないのならば，`std::true_type`から派生する．
